...
function sidebar(categories,active){
  const seasonLinks = Object.entries(seasonStructure).map(([seasonSlug,season])=>{
    ...
    return `<div class="season-menu"><a class="category season-parent season-trigger" href="/season/${seasonSlug}">${esc(season.title.replace(' Color Palettes',''))}</a><div class="season-flyout"><div class="flyout-title">${esc(season.title.replace(' Color Palettes',''))}</div>${miniPalette(firstPage.base,'season-preview-square')}${flyout}</div></div>`;
  }).join('');
...
function setupHoverPreviews(){document.querySelectorAll('.preview-menu,.season-menu').forEach(item=>{item.addEventListener('mouseenter',()=>{const rect=item.getBoundingClientRect();const card=item.querySelector('.preview-card,.season-flyout');const h=card&&card.classList.contains('season-flyout')?360:182;let top=rect.top-20;if(top+h>window.innerHeight-16)top=window.innerHeight-h-16;if(top<16)top=16;item.style.setProperty('--preview-top',top+'px');});});}
...
if(localStorage.getItem('theme')==='dark')document.documentElement.setAttribute('data-theme','dark');setupHoverPreviews();mark();
</script>`}
