...
const seasonalPages = {
  'soft-autumn-color-palette': {keyword:'soft autumn color palette', title:'Soft Autumn Color Palette', season:'Autumn', desc:'Warm, muted, earthy colors like camel, olive, dusty peach, moss, and warm cream.', colors:['#D8A67D','#B78B63','#8A8F5A','#CDBB99','#F1E3C6','#A66A4C','#6F6A43','#E8C7A1']},
  'soft-summer-color-palette': {keyword:'soft summer color palette', title:'Soft Summer Color Palette', season:'Summer', desc:'Cool, soft, muted colors like dusty blue, mauve, lavender grey, soft rose, and cool taupe.', colors:['#A9B7C9','#C7A6B8','#D8C7D8','#8FA1B3','#E6DDE3','#9B8FA3','#B7C9C7','#6F7F91']},
  'deep-autumn-color-palette': {keyword:'deep autumn color palette', title:'Deep Autumn Color Palette', season:'Autumn', desc:'Rich warm colors with depth: espresso, auburn, olive, mustard, rust, and deep teal.', colors:['#3B2416','#7A3E22','#9B5A2E','#6F6B2F','#C18A2D','#1F4E45','#5A2D1F','#D6B26A']},
  'dark-autumn-color-palette': {keyword:'dark autumn color palette', title:'Dark Autumn Color Palette', season:'Autumn', desc:'Deep, warm autumn colors with earthy contrast: chocolate, forest, burnt orange, and antique gold.', colors:['#2B1A12','#4B2E1F','#7C3F1D','#A65F2B','#3F4A25','#6B6A32','#C79A3B','#E1C16E']},
  'deep-winter-color-palette': {keyword:'deep winter color palette', title:'Deep Winter Color Palette', season:'Winter', desc:'Cool, high contrast winter colors: black, icy white, burgundy, emerald, royal blue, and deep purple.', colors:['#050505','#F5F7FA','#6D0F2B','#004D40','#0B3D91','#2A1458','#B00020','#C9D6DF']},
  'winter-color-palette': {keyword:'winter color palette', title:'Winter Color Palette', season:'Winter', desc:'Clear, cool, crisp winter shades with strong contrast and jewel tones.', colors:['#000000','#FFFFFF','#003B73','#C1121F','#0077B6','#6A0572','#00A6A6','#DDE7F0']},
  'autumn-color-palette': {keyword:'autumn color palette', title:'Autumn Color Palette', season:'Autumn', desc:'Warm seasonal palette with burnt orange, mustard, olive, brown, cream, and terracotta.', colors:['#BC6C25','#DDA15E','#606C38','#283618','#FEFAE0','#9C6644','#7F5539','#E6CCB2']},
  'light-spring-color-palette': {keyword:'light spring color palette', title:'Light Spring Color Palette', season:'Spring', desc:'Light, warm, delicate spring colors like peach, fresh mint, ivory, coral, and sunny yellow.', colors:['#FFE5B4','#FFD6A5','#FFB5A7','#FFF3B0','#C1FBA4','#B8F2E6','#FFCAD4','#FFF8E7']},
  'light-summer-color-palette': {keyword:'light summer color palette', title:'Light Summer Color Palette', season:'Summer', desc:'Light, cool, airy colors with soft blue, pink, lavender, pearl, and mist.', colors:['#EAF4FF','#CFE8F3','#F6DDE8','#DCCEF2','#E9EEF5','#BFD7EA','#F7F2F7','#AFCBFF']},
  'warm-autumn-color-palette': {keyword:'warm autumn color palette', title:'Warm Autumn Color Palette', season:'Autumn', desc:'Golden warm autumn colors: pumpkin, camel, mustard, olive, copper, and warm ivory.', colors:['#C76F2D','#D99A3D','#B88746','#8A7A2D','#A65F2B','#F2D6A2','#6B4E2E','#E8B86D']},
  'true-spring-color-palette': {keyword:'true spring color palette', title:'True Spring Color Palette', season:'Spring', desc:'Warm, clear, sunny spring colors with coral, yellow, green, aqua, and peach.', colors:['#FFB703','#FFD166','#FB8500','#FF6B6B','#52B788','#4ECDC4','#FFF3B0','#FFCAD4']},
  'bright-winter-color-palette': {keyword:'bright winter color palette', title:'Bright Winter Color Palette', season:'Winter', desc:'High contrast, electric winter colors: black, white, hot pink, cobalt, emerald, and icy blue.', colors:['#000000','#FFFFFF','#F72585','#3A0CA3','#4361EE','#00B4D8','#00A878','#E0FBFC']}
};
...
function renderSeasonal(slug,categories,palettes){
  const page = seasonalPages[slug];
  if(!page) return null;
  const title = `${page.title} - HEX Colors, Outfit & Design Inspiration`;
  const desc = `${page.desc} Includes curated HEX colors, matching palettes, and related seasonal color ideas.`;
  const canonical = `${SITE_URL.replace(/\/$/,'')}/seasonal/${slug}`;
  const related = palettes.slice(0,12);
  const jsonLd = {'@context':'https://schema.org','@type':'FAQPage','mainEntity':[
    ...
    {'@type':'Question','name':'What HEX colors are in this palette?','acceptedAnswer':{'@type':'Answer','text':`The main HEX colors include ${page.colors.join(', ')}.`}},
    {'@type':'Question','name':'How can I use this color palette?','acceptedAnswer':{'@type':'Answer','text':'Use it for outfits, branding, website design, UI palettes, mood boards, and creative projects.'}}
  ]};
  return `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)}</title><meta name="description" content="${esc(desc)}"><link rel="canonical" href="${esc(canonical)}"><meta property="og:title" content="${esc(title)}"><meta property="og:description" content="${esc(desc)}"><script type="application/ld+json">${jsonEsc(jsonLd)}</script><style>${css()}</style></head><body>${sidebar(categories,null)}<main class="main"><nav class="breadcrumb"><a href="/">Home</a> › Seasonal Colors › ${esc(page.title)}</nav><h1>${esc(page.title)}</h1><p class="subtitle">${esc(desc)}</p><div class="detail-layout"><section><div class="large-palette">${page.colors.slice(0,4).map(c=>`<div class="color" style="background:${c}"><span>${c}</span></div>`).join('')}</div></section><section><h2 class="section-title">HEX Colors</h2><div class="colors-list">${page.colors.map(c=>`<div class="color-row"><div class="swatch"><span class="dot" style="background:${c}"></span>${c}</div><span class="copy-small" onclick="copyText('${c}',this)">Copy</span></div>`).join('')}</div><div class="actions"><button class="action" onclick="copyText('${page.colors.join('\\n')}',this)">Copy Full Seasonal Palette</button></div></section></div><div class="ad-slot">Advertisement Space — seasonal page banner</div><h2 class="section-title">About ${esc(page.title)}</h2><p class="subtitle">${esc(page.title)} focuses on ${esc(page.desc.toLowerCase())} These colors are useful for personal color analysis, outfit planning, branding, website themes, UI design, and mood boards.</p><h2 class="section-title">Related Color Palettes</h2><div class="related-grid">${related.map(p=>`<a href="/palette/${p.slug}"><div class="related-palette">${p.colors.map(c=>`<div style="background:${c}"></div>`).join('')}</div><div class="related-name">${esc(p.name)}</div></a>`).join('')}</div><h2 class="section-title">More Seasonal Color Palettes</h2><div class="related-grid">${Object.entries(seasonalPages).filter(([s])=>s!==slug).map(([s,p])=>`<a href="/seasonal/${s}" class="related-name">${esc(p.title)}</a>`).join('')}</div></main><div class="copy-toast" id="copyToast">Copied</div>${pageScript('')}</body></html>`;
}
...
module.exports = async function handler(req,res){
  ...
  if(req.method === 'GET' && route === 'sitemap.xml'){
    const urls = [`${SITE_URL}/`,...categories.map(c=>`${SITE_URL}/category/${c.toLowerCase()}`),...Object.keys(seasonalPages).map(s=>`${SITE_URL}/seasonal/${s}`),...palettes.map(p=>paletteUrl(p.slug))];
  ...
  if(req.method === 'GET' && route.startsWith('seasonal/')){
    ...
    const page = renderSeasonal(slug,categories,palettes);
