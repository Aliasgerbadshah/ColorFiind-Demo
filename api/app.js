...
function sidebar(categories,active){
  const seasonLinks = Object.entries(seasonStructure).map(([seasonSlug,season])=>{
    ...
    return `<div class="season-menu" data-season="${seasonSlug}"><a class="category season-parent season-trigger preview-menu" href="/season/${seasonSlug}">${esc(season.title.replace(' Color Palettes',''))}${previewCard(firstPage.base)}</a><div class="season-sublist">${subs}</div></div>`;
  }).join('');
