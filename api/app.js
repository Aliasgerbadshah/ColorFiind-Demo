...
function sidebar(categories,active){
  const seasonLinks = Object.entries(seasonStructure).map(([seasonSlug,season])=>{
    ...
    return `<div class="season-menu"><a class="category season-parent season-trigger preview-menu" href="/season/${seasonSlug}" onclick="toggleSeasonSub(event,this)">${esc(season.title.replace(' Color Palettes',''))}${previewCard(firstPage.base)}</a><div class="season-sublist"><a class="sub-season preview-menu" href="/season/${seasonSlug}">All ${esc(season.title.replace(' Color Palettes',''))}${previewCard(firstPage.base)}</a>${subs}</div></div>`;
  }).join('');
...
function toggleSeasonSub(e,el){e.preventDefault();const menu=el.closest('.season-menu');document.querySelectorAll('.season-menu.open').forEach(m=>{if(m!==menu)m.classList.remove('open')});menu.classList.toggle('open');}
