...
const COLORFIIND_BUILD_VERSION = 'clean-final-anchor-cards-v3';
...
function iconLink(){return '<svg class="icon" viewBox="0 0 24 24"><path d="M10 13a5 5 0 0 0 7 0l3-3a5 5 0 0 0-7-7l-1 1"/><path d="M14 11a5 5 0 0 0-7 0l-3 3a5 5 0 0 0 7 7l1-1"/></svg>'}
function iconCopy(){return '<svg class="icon" viewBox="0 0 24 24"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>'}
function card(p){return '<a class="card" href="/palette/'+p.slug+'" data-slug="'+p.slug+'"><div class="palette">'+p.colors.map((c,i)=>'<div class="color c'+(i+1)+'" style="background:'+c+'"><span>'+c+'</span></div>').join('')+'</div><div class="info"><div class="name">'+p.name+'</div><div class="actions"><span class="favorite active">'+iconHeart()+' <span class="like-count">'+(p.likes||0)+'</span></span><span onclick="event.preventDefault();event.stopPropagation();copyText(location.origin+\'/palette/'+p.slug+'\',this)">'+iconLink()+'</span><span onclick="event.preventDefault();event.stopPropagation();copyText(\''+p.colors.join('\\n')+'\',this)">'+iconCopy()+'</span><span onclick="event.preventDefault();event.stopPropagation();downloadPalette(\''+p.name.replace(/'/g,'')+'\','+JSON.stringify(p.colors).replace(/"/g,'&quot;')+',this)">⬇</span></div></div></a>'}
async function copyText(t,el){try{await navigator.clipboard.writeText(t)}catch(e){}let toast=document.getElementById('copyToast');if(toast){toast.textContent='Copied';toast.classList.add('show');setTimeout(()=>toast.classList.remove('show'),1500)}if(el){const o=el.innerHTML;el.innerHTML='✓';setTimeout(()=>el.innerHTML=o,1200)}}
...
fetch('/api/palettes').then(r=>r.json()).then(data=>{const likedSet=new Set(liked);const items=data.filter(p=>likedSet.has(p.slug));grid.innerHTML=items.length?items.map(card).join(''):'<p class="subtitle">No liked palettes yet. Go back to the homepage, category pages, or season pages and click the heart icon to build your collection.</p>';}).catch(()=>{grid.innerHTML='<p class="subtitle">Could not load collection. Please refresh.</p>'});
loadTheme();
</script></body></html>`;
}
