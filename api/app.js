...
function createSeasonPalette(pageSlug,page,index){
  ...
  const termB = ['Whisper','Glow','Love','Mood','Dream','Story','Haze','Field','Garden','Muse','Dawn','Charm'][Math.floor(index / page.terms.length) % 12];
  const name = `${page.title.replace(' Color Palettes','')} ${termA} ${termB}`;
  const offset = index % base.length;
  const amount = ((index % 5) - 2) * 8;
  const colors = [base[offset],base[(offset+2)%base.length],base[(offset+4)%base.length],base[(offset+6)%base.length]].map((c,i)=>shiftHex(c,amount + i*3));
  return {name,category:page.parent ? page.parent.charAt(0).toUpperCase()+page.parent.slice(1) : 'Seasonal',season:page.parent,seasonPage:pageSlug,concept:`${page.keyword} ${termA.toLowerCase()} ${termB.toLowerCase()}`,likes:0,slug:slugifyName(name),colors,source:'seasonal'};
}

function generateSeasonalPalettes(){
  const output=[];
  Object.entries(seasonalPages).forEach(([slug,page])=>{
    for(let i=0;i<48;i++) output.push(createSeasonPalette(slug,page,i));
  });
  return output;
}
...
async function copyText(t,el){try{await navigator.clipboard.writeText(t)}catch(e){}let toast=document.getElementById('copyToast');if(toast){toast.textContent='Copied';toast.classList.add('show');setTimeout(()=>toast.classList.remove('show'),1500)}if(el){const o=el.innerHTML;el.innerHTML='✓ Copied';setTimeout(()=>el.innerHTML=o,1200)}}function downloadPalette(name,colors,el){const canvas=document.createElement('canvas');canvas.width=1200;canvas.height=720;const ctx=canvas.getContext('2d');ctx.fillStyle='#ffffff';ctx.fillRect(0,0,1200,720);ctx.fillStyle='#111';ctx.font='800 54px Inter, Arial';ctx.fillText(name,60,86);const w=1080/colors.length;colors.forEach((c,i)=>{ctx.fillStyle=c;ctx.fillRect(60+i*w,130,w,360);ctx.fillStyle='#111';ctx.font='700 26px Inter, Arial';ctx.fillText(c,70+i*w,540)});ctx.font='800 30px Inter, Arial';ctx.fillText('Color Fiind',60,650);const a=document.createElement('a');a.download=name.toLowerCase().replace(/[^a-z0-9]+/g,'-')+'-color-palette.png';a.href=canvas.toDataURL('image/png');a.click();copyText('Downloaded',el)}function toggleTheme(){const cur=document.documentElement.getAttribute('data-theme');if(cur==='dark'){document.documentElement.removeAttribute('data-theme');localStorage.setItem('theme','light')}else{document.documentElement.setAttribute('data-theme','dark');localStorage.setItem('theme','dark')}}if(localStorage.getItem('theme')==='dark')document.documentElement.setAttribute('data-theme','dark');mark();</script>`}
...
function paletteGridCard(p){
  return `<div class="related-card" data-slug="${p.slug}"><div class="related-palette">${p.colors.map(c=>`<div style="background:${c}"></div>`).join('')}</div><div class="related-name">${esc(p.name)}</div><div class="actions" style="margin-top:8px"><button class="action" onclick="event.stopPropagation();downloadPalette('${esc(p.name)}',${esc(JSON.stringify(p.colors))},this)">⬇ Download</button><a class="action" href="/palette/${p.slug}">Open</a></div></div>`;
}
...
function renderSeasonMainPage(seasonSlug,categories,palettes){
  ...
  const seasonPalettes = palettes.filter(p=>p.season === seasonSlug).slice(0,96);
...
function renderSeasonSubPage(seasonSlug,pageSlug,categories,palettes){
  ...
  const pagePalettes = palettes.filter(p=>p.seasonPage === pageSlug).slice(0,96);
