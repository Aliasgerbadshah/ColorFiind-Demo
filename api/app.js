...
const COLORFIIND_BUILD_VERSION = 'clean-final-anchor-cards-v4';
...
const L='colorfiind-liked-palettes',V='colorfiind-visitor-id',D='colorfiind-liked-palette-data';
...
function likedData(){return JSON.parse(localStorage.getItem(D)||'{}')}
function saveLikedData(data){localStorage.setItem(D,JSON.stringify(data))}
function paletteFromElement(slug,el){const card=el?el.closest('.card'):null;if(card){let colors=[];try{colors=JSON.parse(card.dataset.colors||'[]')}catch(e){}return {slug:card.dataset.slug||slug,name:card.dataset.name||slug,category:card.dataset.category||'',colors,likes:Number((el.querySelector('.like-count')||{}).textContent||0)}}if(window.CURRENT_PALETTE&&window.CURRENT_PALETTE.slug===slug)return window.CURRENT_PALETTE;return null}
function addPaletteData(slug,el){const p=paletteFromElement(slug,el);if(!p)return;const data=likedData();data[slug]={slug:p.slug,name:p.name,category:p.category||'',colors:p.colors||[],likes:Number(p.likes||0),savedAt:Date.now()};saveLikedData(data)}
function removePaletteData(slug){const data=likedData();delete data[slug];saveLikedData(data)}
...
function openActiveSeason(){const parts=location.pathname.split('/');if(parts[1]==='season'&&parts[2]){const menu=document.querySelector('.season-menu[data-season="'+parts[2]+'"]');if(menu)menu.classList.add('open');}}
function setupHoverPreviews(){document.querySelectorAll('.preview-menu,.season-menu').forEach(item=>{item.addEventListener('mouseenter',()=>{let card=null;try{card=item.querySelector(':scope > .preview-card')||item.querySelector(':scope > a > .preview-card')||item.querySelector(':scope > .season-flyout')}catch(e){card=item.querySelector('.preview-card,.season-flyout')}if(!card)return;const rect=item.getBoundingClientRect();const h=card.classList.contains('season-flyout')?Math.min(360,window.innerHeight-32):182;let top=rect.top+(rect.height/2)-(h/2);if(top+h>window.innerHeight-16)top=window.innerHeight-h-16;if(top<16)top=16;card.style.top=top+'px';card.style.left='130px';item.style.setProperty('--preview-top',top+'px')});});}
...
document.addEventListener('click',e=>{const card=e.target.closest('.card');if(card&&card.tagName!=='A'&&!e.target.closest('.actions'))location.href='/palette/'+card.dataset.slug});
...
function paletteGridCard(p){
  ...
  return `<a class="card" href="/palette/${p.slug}" data-slug="${p.slug}" data-name="${esc(p.name)}" data-category="${esc(p.category || '')}" data-colors="${colorsJson}"><div class="palette">${p.colors.map((c,i)=>`<div class="color c${i+1}" style="background:${c}"><span>${c}</span></div>`).join('')}</div><div class="info"><div class="name">${esc(p.name)}</div><div class="actions"><span class="favorite" data-like-slug="${p.slug}" onclick="event.preventDefault();event.stopPropagation();likePalette('${p.slug}',this)">${iconHeart()} <span class="like-count">${count}</span></span><span onclick="event.preventDefault();event.stopPropagation();copyText(location.origin+'/palette/${p.slug}',this)" title="Copy link">${iconLink()}</span><span onclick="event.preventDefault();event.stopPropagation();copyText('${p.colors.join('\\n')}',this)" title="Copy colors">${iconCopy()}</span><span onclick="event.preventDefault();event.stopPropagation();downloadPalette('${esc(p.name)}',${colorsJson},this)" title="Download palette PNG">⬇</span></div></div></a>`;
}
...
function renderCollection(categories){
  return `<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Your Color Palette Collection - ColorFiind</title><link rel="icon" href="${FAVICON_HREF}"><meta name="description" content="View your liked color palettes and saved ColorFiind collection."><style>${css()}</style></head><body>${sidebar(categories,null)}<main class="main"><nav class="breadcrumb"><a href="/">Home</a> › Collection</nav><h1>Your Collection</h1><p class="subtitle">Palettes you liked are saved in this browser. This includes normal palettes, season palettes, and sub-season palettes.</p><div class="ad-slot">Advertisement Space — collection page banner</div><div class="grid" id="collectionGrid"></div></main><div class="copy-toast" id="copyToast">Copied</div><script>
const L='colorfiind-liked-palettes',D='colorfiind-liked-palette-data';
const liked=JSON.parse(localStorage.getItem(L)||'[]');
const stored=JSON.parse(localStorage.getItem(D)||'{}');
const grid=document.getElementById('collectionGrid');
function iconHeart(){return '<svg class="icon" viewBox="0 0 24 24"><path d="M12 21s-7-4.35-9.5-8C.5 10 .9 5.5 5 4c2.3-.8 4.2.2 5 1.5C10.8 4.2 12.7 3.2 15 4c4.1 1.5 4.5 6 2.5 9-2.5 3.65-9.5 8-9.5 8z"/></svg>'}
function iconLink(){return '<svg class="icon" viewBox="0 0 24 24"><path d="M10 13a5 5 0 0 0 7 0l3-3a5 5 0 0 0-7-7l-1 1"/><path d="M14 11a5 5 0 0 0-7 0l-3 3a5 5 0 0 0 7 7l1-1"/></svg>'}
function iconCopy(){return '<svg class="icon" viewBox="0 0 24 24"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>'}
function card(p){const colors=p.colors||[];return '<a class="card" href="/palette/'+p.slug+'" data-slug="'+p.slug+'"><div class="palette">'+colors.map((c,i)=>'<div class="color c'+(i+1)+'" style="background:'+c+'"><span>'+c+'</span></div>').join('')+'</div><div class="info"><div class="name">'+p.name+'</div><div class="actions"><span class="favorite active">'+iconHeart()+' <span class="like-count">'+(p.likes||0)+'</span></span><span onclick="event.preventDefault();event.stopPropagation();copyText(location.origin+\'/palette/'+p.slug+'\',this)">'+iconLink()+'</span><span onclick="event.preventDefault();event.stopPropagation();copyText(\''+colors.join('\\n')+'\',this)">'+iconCopy()+'</span><span onclick="event.preventDefault();event.stopPropagation();downloadPalette(\''+String(p.name).replace(/'/g,'')+'\','+JSON.stringify(colors).replace(/"/g,'&quot;')+',this)">⬇</span></div></div></a>'}
async function copyText(t,el){try{await navigator.clipboard.writeText(t)}catch(e){}let toast=document.getElementById('copyToast');if(toast){toast.textContent='Copied';toast.classList.add('show');setTimeout(()=>toast.classList.remove('show'),1500)}if(el){const o=el.innerHTML;el.innerHTML='✓';setTimeout(()=>el.innerHTML=o,1200)}}
function downloadPalette(name,colors,el){const canvas=document.createElement('canvas');canvas.width=1200;canvas.height=720;const ctx=canvas.getContext('2d');ctx.fillStyle='#fff';ctx.fillRect(0,0,1200,720);ctx.fillStyle='#111';ctx.font='800 54px Arial';ctx.fillText(name,60,86);const w=1080/colors.length;colors.forEach((c,i)=>{ctx.fillStyle=c;ctx.fillRect(60+i*w,130,w,360);ctx.fillStyle='#111';ctx.font='700 26px Arial';ctx.fillText(c,70+i*w,540)});ctx.font='800 30px Arial';ctx.fillText('Color Fiind',60,650);const a=document.createElement('a');a.download=name.toLowerCase().replace(/[^a-z0-9]+/g,'-')+'-color-palette.png';a.href=canvas.toDataURL('image/png');a.click();copyText('Downloaded',el)}
function render(items){grid.innerHTML=items.length?items.map(card).join(''):'<p class="subtitle">No liked palettes yet. Go back to the homepage, category pages, or season pages and click the heart icon to build your collection.</p>'}
function loadTheme(){if(localStorage.getItem('theme')==='dark')document.documentElement.setAttribute('data-theme','dark')}
let items=liked.map(slug=>stored[slug]).filter(Boolean);render(items);fetch('/api/palettes').then(r=>r.json()).then(data=>{const map={};data.forEach(p=>map[p.slug]=p);items=liked.map(slug=>stored[slug]||map[slug]).filter(Boolean).map(p=>map[p.slug]?{...p,likes:map[p.slug].likes}:p);render(items);}).catch(()=>render(items));
loadTheme();
</script></body></html>`;
}
...
module.exports = async function handler(req,res){
  ...
  if(req.method === 'GET' && route === 'api/version') return sendJson(res,200,{version:COLORFIIND_BUILD_VERSION});
