...
function sidebar(categories,active){
  const seasonLinks = Object.entries(seasonStructure).map(([seasonSlug,season])=>{
    ...
    return `<div class="season-menu"><a class="category season-parent season-trigger" href="/season/${seasonSlug}"><span>${esc(season.title.replace(' Color Palettes',''))}</span>${miniPalette(firstPage.base,'season-trigger-preview')}</a><div class="season-flyout"><div class="flyout-title">${esc(season.title.replace(' Color Palettes',''))}</div>${miniPalette(firstPage.base,'season-preview-square')}${flyout}</div></div>`;
  }).join('');
